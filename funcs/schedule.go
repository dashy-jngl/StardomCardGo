...
func ParseCard(url string) (MatchCard, error) {
    ...
    doc.Find("div.match_wrap").Each(func(i int, wrap *goquery.Selection) {
        ...
        if row.Length() > 0 {
            ...
            wrap.Find("div.match_block_columul.match_block_3col").Each(func(_ int, ul *goquery.Selection) {
                var members []string
        ...
        fmt.Println("Match Type:", matchType)
        for i, matches := range matches {
            fmt.Printf("Match %d:\n", i+1)
            for j, team := range matches.Teams {
                fmt.Printf("  Team %d: %s\n", j+1, strings.Join(team, ", "))
            }
        }
    }
    })
